...
allSeq = parseSequencesFromMap(seqmapFile);
...
for s=allSeq
    ...
    %     stI.Xi(find(stI.Xi(:)))=-1;
        ...
        stI.X=zeros(allFgt(seqCnt),0);stI.Y=zeros(allFgt(seqCnt),0);
        stI.Xi=zeros(allFgt(seqCnt),0);stI.Yi=zeros(allFgt(seqCnt),0);
        stI.W=zeros(allFgt(seqCnt),0);stI.H=zeros(allFgt(seqCnt),0);
        stI.Xgp=zeros(allFgt(seqCnt),0);stI.Ygp=zeros(allFgt(seqCnt),0);
        imCoord=1;
        worldCoordST=1;
    end
...
if eval2D
    ...
    allM = reshape([allMets.mets2d.m],14,length(allMets.mets2d))';
    m2d = fastCLEAR(allM, numel(find(gtInfo.Xi)),size(gtInfo.Xi,2),size(gtInfo.Xi,1));
    printMetrics(m2d);
