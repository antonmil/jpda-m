...
for i=1:msp
    ...
    for j=1:SizD
		...
		% S_G
            [dis, inx]= sort(pdist2(Z{1,i} ,(H*X_e(:,j))','mahalanobis',S_G));
            
            TMindx{i}{j}=inx(dis<d_G^.5);
        end
    ...
    if i~=msp
        ...
        Sigma_p=Sigma_u;
        %         Routes_t=Routes;
        %         Measur_indx_t=Measur_indx;
        %         Cost_prob_t=Cost_prob;
        %         clear X_u Sigma_u Routes Measur_indx Cost_prob
        clear X_u Sigma_u
    end
    
end
% Rt_In_Pr(:,:,1)=Routes;
% Rt_In_Pr(:,:,2)=Measur_indx;
% Rt_In_Pr(:,:,3)=Cost_prob;