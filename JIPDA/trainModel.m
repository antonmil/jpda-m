...
ssfile=[confdir,filesep,'ss.txt'];
if exist(ssfile,'file')
    fid=fopen(ssfile);
    searchspace=textscan(fid,'%s');
...
%         params=
    ...
    for scen=allscenes
        ...
        try
            ...
            try
                [metrics2d, metrics3d, stateInfo]=runJIPDA(char(scen),conffile);
            catch err2
...
if bestexper==1 && length(resfiles)==maxexper
    ...
    if rjobs==0
        ...
        if ~exist(newConfdir,'dir')
            ...
            %
            submitstr=sprintf('!ssh moby \"cd research/projects/jpda-m/JIPDA; sh submitTrain.sh %s\"',newSetting)
            fprintf('submit: %s\n',newSetting)
            eval(submitstr);
        end
    else
